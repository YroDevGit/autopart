class CtrTOAST {
    ...
    fire({ text = "", bg = "#333", color = "#fff", icon = "", duration = 3000, effect = "top", click, closed=undefined }) {
        //if(document.getElementById(this.containerId)){
        ...
        if (duration > 0) {
            setTimeout(() => this.removeToast(toast, effect, faded), duration);
        }
    }

    removeToast(toast, effect, faded = undefined) {
        if (!toast) return;
        ...
        setTimeout(() => {
            ...
            if(faded && typeof faded == "function"){
                faded(toast);
            }
        }, 450);
    }
